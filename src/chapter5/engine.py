...
def planner(goal, capabilities, client, generation_model):
    ...
    try:
        ...
        try:
            ...
            logging.error(
                f"Planner failed to parse JSON despite json_mode. Raw String: {plan_json_string}"
            )
        ...
        # Handle edge cases
            ...
            logging.warning(
                "Planner returned a raw list instead of the requested JSON object."
            )
            plan = plan_data
        elif isinstance(plan_data, dict) and "step" in plan_data:
            logging.warning(
                "Planner received a single JSON step object; wrapping it in a list."
            )
            plan = [plan_data]
        else:
            # Addresses the original "The extracted JSON is not a list structure" error.
            logging.error(
                f"Planner returned an unexpected JSON structure. Response: {plan_json_string}"
            )
            raise ValueError(
