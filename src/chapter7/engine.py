...
class ExecutionTrace:
    ...
    def __init__(self, goal):
        ...
        logging.info(f"执行跟踪已初始化，目标: '{self.goal}'")

    def log_plan(self, plan):
        self.plan = plan
        logging.info("计划已记录到跟踪中。")
    ...
    def log_step(self, step_num, agent, planned_input, mcp_output, resolved_input):
        ...
        logging.info(f"步骤 {step_num} ({agent}) 已记录到跟踪中。")
...
def planner(goal, capabilities, client, generation_model):
    ...
    """
    logging.info("规划器已激活，正在分析目标并生成执行计划...")
    ...
    try:
        ...
        if (
            ...
            and isinstance(plan_data["plan"], list)
        ):
            plan = plan_data["plan"]
        ...
        logging.info("规划器成功生成计划。")
        return plan

    except Exception as e:
        logging.error(f"规划器生成有效计划失败。错误: {e}")
        raise e
...
def resolve_dependencies(input_params, state):
    ...
    def resolve(value):
        if isinstance(value, str) and value.startswith("$$") and value.endswith("$$"):
            ref_key = value[2:-2]
            if ref_key in state:
                logging.info(f"执行器已解析依赖项 '{ref_key}'。")
...
def context_engine(
    ...
    """
    logging.info(f"--- 【上下文引擎】开始新任务 --- 目标: {goal}")
    ...
    try:
        ...
        logging.error(f"无法连接到Pinecone索引 '{index_name}': {e}")
    ...
    for step in plan:
        ...
        if not step_num or not agent_name or planned_input is None:
            error_message = f"计划中步骤结构无效: {step}"
            logging.error(f"--- 执行器: 致命错误 --- {error_message}")
            trace.finalize("Failed during Execution (Invalid Plan Structure)")
            return None, trace

        logging.info(f"--- 执行器: 开始步骤 {step_num}: {agent_name} ---")
        ...
        try:
            ...
            logging.info(f"--- 执行器: 步骤 {step_num} 已完成。 ---")

        except Exception as e:
            error_message = f"步骤 {step_num} ({agent_name}) 执行失败: {e}"
            logging.error(f"--- 执行器: 致命错误 --- {error_message}")
